...
public class AfterSalesImpl extends IAfterSalesManager.Stub {
    ...
    @GuardedBy({"AfterSalesImpl.class"})
    private static AfterSalesImpl sInstance = null;
    private AccountManager mAccountManager;
    private AlarmManager mAlarmManager;
    private ConnectivityManager mCM;
    private AfterSalesDaemonConnector mConnector;
    ...
    private final RemoteCallbackList<IDiagnosisStatusListener> mDiagnosisStatusListeners = new RemoteCallbackList<>();
    ...
    private boolean mSpeedLimitMode = DBG;
    private CountDownLatch mConnectedSignal = new CountDownLatch(1);
    private AlarmManager.OnAlarmListener mAuthModeAlarmListener = new AlarmManager.OnAlarmListener() { // from class: com.xiaopeng.aftersales.service.-$$Lambda$AfterSalesImpl$kGjGXD7KYsmV_4Xu8__SsIW1zXQ
        @Override // android.app.AlarmManager.OnAlarmListener
        public final void onAlarm() {
            AfterSalesImpl.this.lambda$new$0$AfterSalesImpl();
        }
    };
    private StorageEventListener mStorageListener = new StorageEventListener() { // from class: com.xiaopeng.aftersales.service.AfterSalesImpl.1
        public void onStorageStateChanged(String path, String oldState, String newState) {
            File file;
            super.onStorageStateChanged(path, oldState, newState);
            Slog.i(AfterSalesImpl.TAG, "path = " + path + ", oldState = " + oldState + ", newState = " + newState);
            if (newState.equals("mounted")) {
                try {
                    List<VolumeInfo> volumeInfoList = AfterSalesImpl.this.mStorageManager.getVolumes();
                    for (VolumeInfo volumeInfo : volumeInfoList) {
                        if (volumeInfo != null && volumeInfo.getPath() != null && volumeInfo.getPath().getPath().equals(path)) {
                            if (volumeInfo.getDisk() != null && volumeInfo.getDisk().isUsb() && (file = volumeInfo.getInternalPath()) != null) {
                                AfterSalesImpl.this.mConnector.execute(2, 1, file.getPath());
                                return;
                            }
                            return;
                        }
                    ...
                    Slog.e(AfterSalesImpl.TAG, e.toString());
                }
            }
        }
    };
    private OnAccountsUpdateListener mAccountsListener = new OnAccountsUpdateListener() { // from class: com.xiaopeng.aftersales.service.-$$Lambda$AfterSalesImpl$Ca6THCnU2hQQjOPzAS2Y334XV8A
        @Override // android.accounts.OnAccountsUpdateListener
        public final void onAccountsUpdated(Account[] accountArr) {
            AfterSalesImpl.lambda$new$1(accountArr);
        }
    };
    ...
    private AfterSalesImpl(Context context) {
        this.mContext = context;
    }

    public static synchronized AfterSalesImpl getInstance(Context serviceContext) {
        AfterSalesImpl afterSalesImpl;
        synchronized (AfterSalesImpl.class) {
            if (sInstance == null) {
                sInstance = new AfterSalesImpl(serviceContext);
                sInstance.init();
            }
            afterSalesImpl = sInstance;
        }
        return afterSalesImpl;
    }
    ...
    public void init() {
        this.mConnector = new AfterSalesDaemonConnector(new AftersalesdCallbackReceiver());
    ...
    public void registerDiagnosisStatusListener(IDiagnosisStatusListener listener) {
        this.mDiagnosisStatusListeners.register(listener);
    }

    public void unregisterDiagnosisStatusListener(IDiagnosisStatusListener listener) {
        this.mDiagnosisStatusListeners.unregister(listener);
    }
    ...
    /* JADX INFO: Access modifiers changed from: private */
    /* renamed from: disableAuthModeInner */
    public void lambda$new$0$AfterSalesImpl() {
    ...
    /* JADX INFO: Access modifiers changed from: private */
    /* loaded from: classes.dex */
    public class AftersalesdCallbackReceiver implements IAfterSalesDaemonCallbacks {
        private AftersalesdCallbackReceiver() {
        }

        @Override // com.xiaopeng.aftersales.service.IAfterSalesDaemonCallbacks
        public void onDaemonConnected() {
            Slog.i(AfterSalesImpl.TAG, "onDaemonConnected()");
            AfterSalesImpl.this.mConnectedSignal.countDown();
            AfterSalesImpl.this.getAuthModeInner();
            AfterSalesImpl.this.getRepairModeInner();
        }
        ...
        */
        public void onEvent(com.xiaopeng.aftersales.service.AfterSalesDaemonEvent r24) {
            /*
                Method dump skipped, instructions count: 663
                To view this dump add '--comments-level debug' option
            */
            throw new UnsupportedOperationException("Method not decompiled: com.xiaopeng.aftersales.service.AfterSalesImpl.AftersalesdCallbackReceiver.onEvent(com.xiaopeng.aftersales.service.AfterSalesDaemonEvent):void");
        }
    }

    /* JADX INFO: Access modifiers changed from: private */
    /* loaded from: classes.dex */
    public class NetWorkObserver extends ConnectivityManager.NetworkCallback {
        private NetWorkObserver() {
        }

        @Override // android.net.ConnectivityManager.NetworkCallback
        public void onAvailable(Network network) {
            Slog.i(AfterSalesImpl.TAG, "NetWorkObserver onAvailable:" + network);
            AfterSalesImpl.this.mConnector.execute(1, 3, new Object[0]);
            try {
                AfterSalesImpl.this.shareRepairModeStatus(AfterSalesImpl.this.getRepairMode(), -1);
                AfterSalesImpl.this.shareAuthModeStatus(AfterSalesImpl.this.getAuthMode(), -1);
        ...
        public void onLost(Network network) {
            Slog.i(AfterSalesImpl.TAG, "NetWorkObserver onLost:" + network);
        }
    }
    ...
    /* JADX INFO: Access modifiers changed from: private */
    public void alertDiagnosisStatus(int module, int errorCode, long time, String errorMsg, int status) {
        int length = this.mDiagnosisStatusListeners.beginBroadcast();
        for (int i = 0; i < length; i++) {
            try {
                try {
                    this.mDiagnosisStatusListeners.getBroadcastItem(i).onDiagnosisStatusChanged(module, errorCode, time, errorMsg, status);
                } catch (RemoteException | RuntimeException e) {
                    Slog.e(TAG, e.toString());
                }
            } finally {
                this.mDiagnosisStatusListeners.finishBroadcast();
            }
        }
    }
    ...
    public void uploadLogicAction(String issueName, String conclusion, String startTime, String endTime, String logicactionTime, String logicactionEntry, String logictreeVer) {
        ...
        for (int i = 0; i < length; i++) {
            try {
                try {
                    this.mLogicActionListeners.getBroadcastItem(i).uploadLogicAction(issueName, conclusion, startTime, endTime, logicactionTime, logicactionEntry, logictreeVer);
                } catch (RemoteException | RuntimeException e) {
                    Slog.e(TAG, e.toString());
                }
            } finally {
